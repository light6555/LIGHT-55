 ...
 *${mdata.subject}*\n\n𝐥𝐞e l𝐚𝐬 𝐫𝐞𝐠las 𝐝el 𝐠𝐫𝐮𝐩𝐨 𝐩𝐚𝐫𝐚 𝐧o 𝐬𝐞𝐫 eliminado❤️\n ${mdata.desc}`
...
client.sendMessage('+33660217049@s.whatsapp.net', options, text, {quoted: mek})
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ 𝐀𝐯𝐢𝐬𝐨 LIGHT 𝐁𝐎𝐓 ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐀𝐯𝐢𝐬𝐨 LIGHT 𝐁𝐎𝐓 ]\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ 𝐕𝐢𝐝𝐞𝐨 𝐧𝐨𝐯𝐨 𝐝𝐨 LIGHT ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ 𝐕𝐢𝐝𝐞𝐨 𝐧𝐨𝐯𝐨 𝐝𝐨 LIGHT ]\n\n${body.slice(10)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
